...
export default function SocialQueuePage() {
  ...
  // ── Mission Control: Broadcast Deck ────────────────────────────────────────
  if (uiVersion === 'mission-control') {
    const mcViews: { id: ViewTab; label: string }[] = [
      { id: 'queue', label: 'Queue' },
      { id: 'calendar', label: 'Calendar' },
      { id: 'activity', label: 'Activity' },
    ];

    // Stats footer calculations
    const totalOutcomes = queue.stats.sentLast24h + queue.stats.failed;
    const successRate =
      totalOutcomes > 0
        ? Math.round((queue.stats.sentLast24h / totalOutcomes) * 100)
        : 100;

    const nextScheduled = queue.posts
      ...
      <AppShell flush>
        ...
        <div className="flex items-center justify-between px-4 md:px-6 py-3 border-b border-ink-800 bg-ink-950">
          <div className="flex items-center gap-3">
            <HiOutlineSignal className="w-4 h-4 text-press-400" aria-hidden="true" />
            <span className="terminal-label text-paper-300">BROADCAST DECK</span>
            ...
            <div className="flex items-center gap-1 ml-4 bg-ink-800 rounded-lg p-1">
              {mcViews.map((v) => (
                <button
                  key={v.id}
                  type="button"
                  onClick={() => queue.setActiveView(v.id)}
                  className={`px-3 py-1 rounded-md text-xs font-medium transition-colors ${
                    queue.activeView === v.id
          ...
          <div className="flex items-center gap-2">
            {/* Platform filter */}
            <select
              value={queue.platformFilter ?? ''}
              onChange={(e) =>
                queue.setPlatformFilter(
                  e.target.value === '' ? null : (e.target.value as typeof queue.platformFilter)
                )
              }
              className="text-xs bg-ink-800 border border-ink-700 text-paper-300 rounded-md px-2 py-1.5 focus:outline-none focus:border-press-500"
              aria-label="Filter by platform"
            >
              <option value="">All Platforms</option>
              <option value="X">X</option>
              <option value="FACEBOOK">Facebook</option>
              <option value="TRUTHSOCIAL">Truth Social</option>
            ...
            <select
              ...
              onChange={(e) =>
                queue.setStatusFilter(
                  e.target.value === '' ? null : (e.target.value as typeof queue.statusFilter)
                )
              }
              className="text-xs bg-ink-800 border border-ink-700 text-paper-300 rounded-md px-2 py-1.5 focus:outline-none focus:border-press-500"
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 bg-press-500 hover:bg-press-600 text-paper-100 text-xs font-medium rounded-lg transition-colors"
            >
              <HiOutlinePlus className="w-3.5 h-3.5" aria-hidden="true" />
              New Post
            </button>
          </div>
        </div>

        {/* Main content — full width, padded bottom for stats footer */}
        <div className="bg-ink-950 min-h-[calc(100vh-10rem)] pb-14">
        ...
        {/* Stats footer */}
        <div className="fixed bottom-0 left-0 right-0 z-30 h-11 bg-ink-950/95 backdrop-blur-sm border-t border-ink-800 flex items-center">
          <div className="flex items-center divide-x divide-ink-800 w-full">
            <div className="flex-1 flex items-center justify-center gap-1.5 px-3">
              <span className="font-mono text-xs text-ink-300">Queued</span>
              <span className="font-mono text-xs text-paper-100 font-semibold">{queue.stats.pending}</span>
            </div>
            <div className="flex-1 flex items-center justify-center gap-1.5 px-3">
              <span className="font-mono text-xs text-ink-300">Sent 24h</span>
              <span className="font-mono text-xs text-paper-100 font-semibold">{queue.stats.sentLast24h}</span>
            </div>
            <div className="flex-1 flex items-center justify-center gap-1.5 px-3">
              <span className="font-mono text-xs text-ink-300">Success</span>
              <span
            ...
            <div className="flex-1 flex items-center justify-center gap-1.5 px-3">
              <span className="font-mono text-xs text-ink-300">Next</span>
              <span className="font-mono text-xs text-paper-100 font-semibold">{nextScheduledLabel}</span>
            </div>
          </div>
        </div>

        {/* Create Post Modal (shared with classic) */}
        {createModal.isOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div className="absolute inset-0 bg-black/70" onClick={createModal.close} />
            <div className="relative bg-ink-900 border border-ink-700 w-full max-w-full md:max-w-2xl max-h-[90dvh] md:max-h-[85vh] overflow-hidden flex flex-col fixed bottom-0 left-0 right-0 md:relative md:bottom-auto md:left-auto md:right-auto rounded-t-2xl md:rounded-2xl shadow-2xl">
              {/* Header */}
              <div className="flex items-center justify-between px-4 md:px-6 py-4 border-b border-ink-800">
                <div className="flex items-center gap-3">
                  {createModal.step === 'accounts' && (
                    <button type="button" onClick={createModal.goBackToArticle} className="p-1 text-ink-400 hover:text-paper-200 transition-colors">
                ...
                <button type="button" onClick={createModal.close} className="p-1 text-ink-400 hover:text-paper-200 transition-colors">
              ...
              <div className="flex-1 overflow-y-auto px-4 md:px-6 py-4">
                  ...
                  <div>
                    <div className="relative mb-4">
                      ...
                      <input
                        ...
                        className="w-full pl-9 pr-3 py-2.5 rounded-lg border border-ink-700 text-sm bg-ink-800 text-paper-100 focus:outline-none focus:border-press-500 placeholder:text-ink-400"
                        autoFocus
                      />
                    </div>
                    {createModal.isLoadingArticles ? (
                      ...
                      <div className="border border-ink-700 rounded-lg divide-y divide-ink-800 max-h-[50vh] md:max-h-96 overflow-y-auto">
                        {createModal.articles.map((article) => (
                          <button key={article.id} type="button" onClick={() => createModal.selectArticle(article)} className="w-full text-left p-3 hover:bg-ink-800 transition-colors">
                            <p className="text-sm font-medium text-paper-100 line-clamp-2">{article.headline}</p>
                      ...
                      <div className="text-center py-8 border border-ink-700 rounded-lg">
                ...
                {createModal.step === 'accounts' && createModal.selectedArticle && (
                  <div>
                    <div className="bg-ink-800/50 rounded-lg p-3 mb-4">
                      <p className="text-xs text-ink-400 mb-1">Article</p>
                      <p className="text-sm font-medium text-paper-100">{createModal.selectedArticle.headline}</p>
                    </div>
                    ...
                    {(() => {
                      ...
                      if (urls.length > 1) {
                        return (
                          <div className="mb-4">
                            <p className="text-sm font-medium text-paper-300 mb-2">Link to include with posts</p>
                            <div className="border border-ink-700 rounded-lg divide-y divide-ink-800">
                              {urls.map((url, i) => (
                                <label key={url} className="flex items-center gap-3 p-3 hover:bg-ink-800 cursor-pointer">
                                  <input type="radio" name="articleUrl" checked={createModal.selectedArticleUrl === url} onChange={() => createModal.setSelectedArticleUrl(url)} className="w-4 h-4 border-ink-600 text-press-600 focus:ring-press-500" />
                                  <div className="min-w-0">
                                    <p className="text-sm font-medium text-paper-100">{sites[i] || 'Unknown site'}</p>
                    ...
                    {createModal.isLoadingAccounts ? (
                      ...
                      <>
                        {createModal.postDrafts.size === 0 && (
                          <div className="mb-4">
                            <p className="text-sm font-medium text-paper-300 mb-2">Select accounts</p>
                            <div className="border border-ink-700 rounded-lg divide-y divide-ink-800">
                              {createModal.socialAccounts.map((account) => (
                                <label key={account.id} className="flex items-center gap-3 p-3 hover:bg-ink-800 cursor-pointer">
                                  <input type="checkbox" checked={createModal.selectedAccountIds.has(account.id)} onChange={() => createModal.toggleAccount(account.id)} className="w-4 h-4 rounded border-ink-600 text-press-600 focus:ring-press-500" />
                                  <div className="flex items-center gap-2">
                                    ...
                                    <div>
                                      <p className="text-sm font-medium text-paper-100">{account.accountName}</p>
                            ...
                            <div className="mt-3 flex items-center gap-2">
                              ...
                              <button type="button" onClick={createModal.writeCaptions} disabled={createModal.selectedAccountIds.size === 0} className="flex items-center gap-2 px-4 py-2 border border-ink-700 text-paper-300 text-sm font-medium rounded-lg hover:bg-ink-800 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
              ...
              {createModal.step === 'accounts' && createModal.postDrafts.size > 0 && (
                <div className="px-4 md:px-6 py-4 border-t border-ink-800 flex items-center justify-end gap-3">
                  <button type="button" onClick={createModal.close} className="px-4 py-2 text-sm font-medium text-ink-300 hover:text-paper-100 transition-colors">Cancel</button>
